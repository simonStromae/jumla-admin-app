  ...
  return (
    <Modal width={820} onClose={onClose}
      ...
      sub={isEdit
        ...
        : 'Fiche client utilisée pour expédier ou recevoir des colis'}
      footer={
        <>
        ...
        <div>
          ...
          <div style={{ marginTop: 18 }}>
            <div className="tabs" style={{ marginBottom: 14 }}>
              ...
              {(data.role === 'sender' || data.role === 'both') && (
                <button className={'tab ' + (tab === 'sender' ? 'is-active' : '')} onClick={() => setTab('sender')}>Expédition · Douala</button>
              )}
              {(data.role === 'recipient' || data.role === 'both') && (
                <button className={'tab ' + (tab === 'recip' ? 'is-active' : '')} onClick={() => setTab('recip')}>Réception · Canada</button>
              )}
                ...
                {data.role === 'recipient' && (
                  <div className="field" style={{ marginBottom: 0 }}>
                    ...
                    <select className="select" value={data.linkedClient} onChange={e => upd('linkedClient', e.target.value)}>
                      ...
                      {DATA.CLIENTS.filter(c => c.role === 'sender' || c.role === 'both').map(c => (
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              ...
              <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border-soft)', background: 'var(--bg-soft)', fontSize: 11.5, color: 'var(--ink-500)', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {isEdit
                  ...
                  : 'Le client sera disponible pour les nouveaux colis'}
              </div>
            </div>
          </div>
        </div>
