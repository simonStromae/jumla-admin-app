  ...
  return (
    <div>
      ...
      <div className="field-row field-row--3">
        {[
          { label: 'Dépassement de poids', key: 'overrunRate', suffix: `${data.currency}/kg` },
          { label: 'Livraison à domicile', key: 'deliveryFee', suffix: data.currency },
          { label: 'Frais de manutention', key: 'handlingFee', suffix: data.currency },
...
function StepCosts({ data, upd }) {
  ...
  return (
    <div>
      ...
      <div className="field-row field-row--3">
        {[
          { label: 'Transport aérien', key: 'internalTransport' },
          { label: 'Douane & dédouanement', key: 'internalCustoms' },
          { label: 'Entrepôt & logistique', key: 'internalWarehouse' },
        ].map(({ label, key }) => (
          <div key={key} className="field">
            <label className="label">{label}</label>
            <div style={{ position: 'relative' }}>
      ...
      <div style={{ marginTop: 16, padding: 18, background: 'linear-gradient(135deg, var(--ink-900), var(--ink-800))', color: 'white', borderRadius: 10 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {[
            { l: 'CA estimé',          v: estRevenue, sign: '' },
            { l: 'Coûts totaux',       v: total,      sign: '−' },
            { l: 'Marge prévisionnelle', v: margin,   sign: '+', gold: true },
          ].map(({ l, v, sign, gold }, i) => (
            <div key={i}>
    ...
    <div>
      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
        ...
        <div style={{ padding: 18, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
          {[
            { label: 'Route',            value: `${route?.fromCity} → ${route?.toCity}` },
            { label: 'Transit',          value: `${route?.transitDays} jours` },
            { label: 'Départ',           value: data.depDate },
            { label: 'Arrivée estimée',  value: data.arrDate },
            { label: 'Capacité max',     value: `${data.capacityMax.toLocaleString('fr')} kg` },
            { label: 'Capacité réservée', value: `${data.capacityReserved.toLocaleString('fr')} kg` },
          ].map(({ label, value }) => (
            <div key={label}>
