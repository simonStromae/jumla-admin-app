export function Pagination({ total, page, pageSize, onPageChange, onPageSizeChange, sizes = [10, 25, 50, 100] }) {
    ...
    <div style={{
      ...
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>
        <button className="icon-btn" disabled={page <= 1} onClick={() => onPageChange(1)}
          style={page <= 1 ? { opacity: 0.3, cursor: 'not-allowed' } : {}} title="Première page">
        ...
        <button className="icon-btn" disabled={page <= 1} onClick={() => onPageChange(page - 1)}
          style={page <= 1 ? { opacity: 0.3, cursor: 'not-allowed' } : {}} title="Précédent">
        ...
        {pagesToShow.map((p, i) =>
          p === '…'
            ...
            : <button key={i} onClick={() => onPageChange(p)} className="mono" style={{
        ...
        <button className="icon-btn" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)}
          style={page >= totalPages ? { opacity: 0.3, cursor: 'not-allowed' } : {}} title="Suivant">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6" /></svg>
        </button>
        <button className="icon-btn" disabled={page >= totalPages} onClick={() => onPageChange(totalPages)}
          style={page >= totalPages ? { opacity: 0.3, cursor: 'not-allowed' } : {}} title="Dernière page">
