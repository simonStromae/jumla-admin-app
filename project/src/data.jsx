...
// Parcel categories — global defaults; route-level overrides go in ROUTES[i].categories
...
window.DATA = { ROUTES, CAMPAIGNS, PARCELS, CLIENTS, PAYMENTS, AGENTS, PARCEL_CATEGORIES };
window.getRoute = (id) => ROUTES.find(r => r.id === id);
window.getCampaign = (id) => CAMPAIGNS.find(c => c.id === id);
window.getCategory = (id) => PARCEL_CATEGORIES.find(c => c.id === id);
