// ============================================
// ZENDIT — Campaign status workflow + transition modal
// ============================================

const STATUS_STEPS = [
  { id: 'draft',     label: 'Brouillon',   en: 'Draft',      icon: 'FileText', desc: 'Cargaison en préparation. Aucune visibilité client.' },
  { id: 'open',      label: 'Ouverte',     en: 'Open',       icon: 'Plus',     desc: 'Accepte des colis. Clients informés à l\'ouverture.' },
  { id: 'locked',    label: 'Verrouillée', en: 'Locked',     icon: 'Lock',     desc: 'Plus de nouveaux colis. Prête pour expédition.' },
  { id: 'in-transit',label: 'En transit',  en: 'In transit', icon: 'Plane',    desc: 'Expédition en cours. Édition restreinte.' },
  { id: 'arrived',   label: 'Arrivée',     en: 'Arrived',    icon: 'Pin',      desc: 'À destination. Pointage et livraisons.' },
  { id: 'closed',    label: 'Clôturée',    en: 'Closed',     icon: 'Check',    desc: 'Cargaison archivée avec bilan financier.' },
];
...
const TRANSITIONS = {
  ...
  'locked → in-transit': {
    ...
    requires: [
      { ok: true, label: 'Manifeste signé par l\'agent origine' },
    ],
    cta: 'Confirmer le départ',
    tone: 'info',
    needsDate: true,
  },
  'in-transit → arrived': {
    ...
    effects: [
      ...
      { kind: 'system',   what: 'Compteur "délai post-arrivée" démarré' },
    ],
    requires: [
      { ok: true, label: 'Agent destination présent à l\'entrepôt' },
    ],
    cta: 'Marquer comme arrivée',
    tone: 'info',
    needsDate: true,
  },
  'arrived → closed': {
    title: 'Clôturer la cargaison',
    description: 'Bilan financier final, archivage et stats. Aucune notification client (les destinataires ont déjà été notifiés au transit et à l\'arrivée).',
    effects: [
      ...
      { kind: 'auto',     what: 'Reclassement automatique des impayés (report / perte)' },
    ],
    requires: [
      { ok: true,  label: 'Tous les bordereaux validés (64/64)' },
      { ok: false, label: '12 impayés à régler ou requalifier', action: 'Voir les impayés' },
    ],
    cta: 'Clôturer définitivement',
    tone: 'primary',
  },
};

// === Status stepper (mini, for header) ===
function StatusStepperMini({ current, onClick }) {
...
// === Status panel (large, in detail page) ===
function StatusPanel({ status, onAdvance, onJump }) {
...
// === Transition modal — shows consequences before confirming ===
function StatusTransitionModal({ from, to, onClose, onConfirm }) {
  const key = from + ' → ' + to;
  ...
  return (
    <Modal width={620} onClose={onClose}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {t.requires.map((r, i) => (
              <div key={i} style={{
                ...
                {r.ok
                  ? <I.Check style={{ color: 'var(--ok-600)', width: 16, height: 16 }} />
                  : <I.Alert style={{ color: 'var(--bad-600)', width: 16, height: 16 }} />}
      ...
      {/* Date input for transitions that need it */}
      {t.needsDate && (
...
window.StatusStepperMini = StatusStepperMini;
window.StatusPanel = StatusPanel;
window.StatusTransitionModal = StatusTransitionModal;
