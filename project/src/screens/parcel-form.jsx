// ============================================
// ZENDIT — Modal: Nouveau / Édition colis
// ============================================

function ParcelFormModal({ mode = 'create', parcel, campaign, onClose, onSave }) {
  const isEdit = mode === 'edit';
  const route = window.getRoute(campaign?.route) || window.DATA.ROUTES[0];
  ...
  const [data, setData] = useState(() => ({
    ...
    items: parcel?.items || (parcel ? [
      { id: 1, name: 'Valise médium — vêtements adulte', packs: 1, pieces: 1, value: 800, note: '' },
      { id: 2, name: 'Valise large — chaussures, sacs',  packs: 1, pieces: 1, value: 1200, note: '' },
      { id: 3, name: 'Carton — ndolè, épices, café',     packs: 1, pieces: 8, value: 320, note: '8 sachets' },
    ] : [
      { id: 1, name: '', packs: 1, pieces: 1, value: 0, note: '' },
    ]),
    contentValue: parcel?.contentValue || 0,
  ...
  // Category surcharge
  const category = window.getCategory(data.category) || window.DATA.PARCEL_CATEGORIES[0];
    ...
    <Modal width={920} onClose={onClose}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 22 }}>
        ...
        <div>
          ...
          {/* Recipient */}
          <PartyField
            kind="Destinataire · Canada"
            en="Recipient"
            iconColor="var(--info-600)"
            color={2}
            data={data}
            prefix="recip"
            cityLabel={data.recipCity}
            upd={upd}
            mode="recipients"
            withAddress
          />
          ...
          <div className="card" style={{ padding: 16, marginBottom: 14 }}>
            ...
            <div className="field">
              <label className="label">Note générale sur le contenu <span className="opt">/ General note</span></label>
              <textarea className="textarea" rows={2} value={data.contents} onChange={e => upd('contents', e.target.value)} placeholder="Description résumée, instructions de manutention, particularités..." />
            </div>
            <div className="field" style={{ marginBottom: 0 }}>
          ...
          <div className="card" style={{ padding: 16, marginBottom: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <DeliveryCard sel={data.delivery==='home'} onClick={() => upd('delivery', 'home')}
                icon={<I.Truck style={{width:18, height:18}}/>} label="Livraison à domicile" en="Home delivery" extra={`+${25} ${route.currency}`} />
              <DeliveryCard sel={data.delivery==='pickup'} onClick={() => upd('delivery', 'pickup')}
                icon={<I.Warehouse style={{width:18, height:18}}/>} label="Retrait entrepôt" en="Warehouse pickup" extra="Gratuit" />
            </div>
          </div>
            ...
            <div className="field-row field-row--3">
              ...
              <div className="field">
                ...
                <select className="select" value={data.method} onChange={e => upd('method', e.target.value)}>
                  ...
                  <option>Crédit / Acompte</option>
                </select>
              </div>
              <div className="field">
                <label className="label">Agent en charge</label>
                <select className="select" value={data.agent} onChange={e => upd('agent', e.target.value)}>
                  {window.DATA.AGENTS.map(a => <option key={a.id} value={a.initials}>{a.initials} — {a.name}</option>)}
                </select>
              </div>
            </div>
            <div className="field" style={{ marginBottom: 0 }}>
              <label className="label">Observations <span className="opt">/ Notes</span></label>
              <input className="input" value={data.note} onChange={e => upd('note', e.target.value)} placeholder="Note interne, indications particulières..." />
            </div>
          </div>
        </div>
          ...
          <div style={{ position: 'sticky', top: 0 }}>
            <div className="card" style={{ overflow: 'hidden' }}>
              <div style={{ padding: '14px 16px', background: 'linear-gradient(135deg, var(--ink-900), var(--ink-800))', color: 'white' }}>
                ...
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,.55)' }}>
                  {campaign?.code || 'DLA-YUL-APR-02'} · Aperçu en direct
                </div>
              </div>

              <div style={{ padding: 16 }}>
                <div style={{ fontSize: 10.5, color: 'var(--ink-400)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 10 }}>
                  Détail du tarif
                </div>
                ...
                <div style={{ borderTop: '1px solid var(--border)', marginTop: 10, paddingTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                  <span style={{ fontWeight: 700 }}>Total dû</span>
                  <span className="mono" style={{ fontSize: 22, fontWeight: 700, whiteSpace: 'nowrap' }}>
              ...
              <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border-soft)', background: 'var(--bg-soft)' }}>
                <div style={{ fontSize: 10.5, color: 'var(--ink-400)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', marginBottom: 6 }}>
                  Grille — route {route.code.replace('→','→')}
                </div>
            ...
            <div className="card" style={{ marginTop: 12, padding: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12 }}>
                <Auto checked label="Générer le bordereau (BL)" />
                <Auto checked label="Envoyer la facture par WhatsApp" />
                <Auto label="Notifier l'expéditeur" />
              </div>
            </div>
          </div>
        </div>
...
function calcPrice(weight, grid) {
  ...
  for (const tier of grid) {
    if (weight > tier.from && weight <= tier.to) {
      return Math.round(weight * tier.rate);
    }
  }
  // Heavier than last tier
  const last = grid[grid.length - 1];
  return Math.round(weight * last.rate);
}
...
function PartyField({ kind, en, iconColor, color, data, prefix, cityLabel, upd, mode, withAddress }) {
  ...
  return (
    <div className="card" style={{ padding: 16, marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 10 }}>
        <div className="section-title" style={{ margin: 0, whiteSpace: 'nowrap' }}>
          <I.Pin style={{ width: 14, height: 14, color: iconColor, flex: '0 0 14px' }} />
          <span>{kind}</span>
          <span style={{ color: 'var(--ink-300)', fontWeight: 500 }}>/ {en}</span>
        </div>
        <button className="btn btn--ghost btn--sm" style={{whiteSpace:'nowrap'}}><I.Search />Choisir existant</button>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: 10, background: name ? 'var(--bg-soft)' : 'transparent', border: name ? 'none' : '1px dashed var(--border)', borderRadius: 8, marginBottom: 12 }}>
        {name ? (
          ...
          <div style={{ flex: 1, color: 'var(--ink-400)', fontSize: 12.5, padding: '6px 0' }}>
            Aucun client sélectionné — renseignez les champs ci-dessous pour créer un nouveau client.
          </div>
        )}
      </div>
          ...
          <div className="field-row field-row--2" style={{ marginBottom: 14 }}>
            <div className="field" style={{marginBottom:0}}>
              ...
              <select className="select" value={data.recipCity} onChange={e => upd('recipCity', e.target.value)}>
                <option>Montréal</option><option>Laval</option><option>Longueuil</option><option>Brossard</option><option>Gatineau</option><option>Québec</option><option>Toronto</option>
              </select>
            </div>
            <div className="field" style={{marginBottom:0}}>
              <label className="label">Langue préférée <span className="opt">/ Language</span></label>
...
function DeliveryCard({ sel, onClick, icon, label, en, extra }) {
  return (
    <label onClick={onClick} style={{
      padding: 14,
      border: '1px solid ' + (sel ? 'var(--brand-500)' : 'var(--border)'),
      background: sel ? 'var(--brand-50)' : 'white',
      borderRadius: 8, cursor: 'pointer',
      display: 'flex', alignItems: 'center', gap: 10,
      color: sel ? 'var(--brand-700)' : 'var(--ink-700)',
    }}>
...
function ToggleChip({ checked, onChange, icon, children }) {
  return (
    <button onClick={onChange} style={{
      ...
      color: checked ? 'white' : 'var(--ink-600)',
      border: '1px solid ' + (checked ? 'var(--brand-500)' : 'var(--border)'),
    ...
    <div className="card" style={{ padding: 0, marginBottom: 14, overflow: 'hidden' }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11.5, color: 'var(--ink-400)', marginTop: 2 }}>Détaillez chaque article — cette liste alimente le bordereau de vérification.</div>
        </div>
        <div style={{ display: 'flex', gap: 14, fontSize: 11.5 }}>
          <MiniStat label="Articles" v={items.length} />
          <MiniStat label="Colis" v={totalPacks} />
          ...
          {items.map((it, idx) => (
            <tr key={it.id}>
              ...
              <td style={{ padding: '6px 8px' }}>
                <input className="input input--sm" value={it.name} onChange={e => updRow(it.id, 'name', e.target.value)}
                  placeholder="Ex: Valise — vêtements adulte" list={`items-presets-${it.id}`} />
                <datalist id={`items-presets-${it.id}`}>
              ...
              <td style={{ padding: '6px 8px', whiteSpace: 'nowrap' }}>
                <button className="icon-btn" title="Dupliquer la ligne" onClick={() => duplicateRow(it)} style={{ width: 26, height: 26 }}>
                  <I.Copy style={{ width: 13, height: 13 }} />
                </button>
                <button className="icon-btn" title="Supprimer la ligne" onClick={() => removeRow(it.id)} disabled={items.length <= 1} style={{ width: 26, height: 26, color: items.length <= 1 ? 'var(--ink-300)' : 'var(--bad-500)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--bg-soft)', borderTop: '1px solid var(--border-soft)' }}>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          <button className="btn btn--ghost btn--sm"><I.Copy />Importer depuis un colis précédent</button>
        </div>
        <div style={{ fontSize: 11.5, color: 'var(--ink-500)' }}>
          Total ligne par ligne : <strong className="mono" style={{ color: 'var(--ink-800)' }}>{totalPieces} pièces</strong> · valeur déclarée <strong className="mono" style={{ color: 'var(--ink-800)' }}>{totalValue.toLocaleString('fr')} {currency}</strong>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="card" style={{ padding: 16, marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10, gap: 10 }}>
        ...
        <span style={{ fontSize: 11.5, color: 'var(--ink-400)' }}>Une catégorie ajoute un % à la grille tarifaire de la route.</span>
      </div>
        ...
        {showOverride && (
          <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid ' + selectedCat.color + '33', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontSize: 11.5, color: 'var(--ink-400)' }}>
              Défaut catégorie : <strong className="mono">{selectedCat.pct > 0 ? '+' : ''}{selectedCat.pct}%</strong> · Hérité de la route
            </span>
...
function SummaryLine({ l, sub, v, cur, warn }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '5px 0', fontSize: 12.5, gap: 8 }}>
      <span style={{ color: warn ? 'var(--warn-700)' : 'var(--ink-600)', minWidth: 0, flex: 1 }}>
        {l}
        {sub && <span style={{ color: 'var(--ink-300)', marginLeft: 4, fontSize: 11 }}>· {sub}</span>}
      </span>
      <span className="mono" style={{ fontWeight: 600, whiteSpace: 'nowrap', flex: '0 0 auto' }}>{v} <span style={{ color: 'var(--ink-400)', fontWeight: 500, fontSize: 11 }}>{cur}</span></span>
    </div>
  );
}

function Auto({ checked, label }) {
  return (
...
window.ParcelFormModal = ParcelFormModal;
