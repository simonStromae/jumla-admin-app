// ============================================
// ZENDIT — Modal: Agent create/edit + permissions
// ============================================

// Permission matrix definition — grouped by module
const PERMISSION_MODULES = [
  {
    id: 'cargaisons', label: 'Cargaisons', icon: 'Plane',
    perms: [
  ...
  {
    id: 'parcels', label: 'Colis', icon: 'Box',
    perms: [
      { id: 'view',   label: 'Voir' },
...
function AgentFormModal({ mode = 'create', agent, onClose, onSave }) {
    ...
    <Modal width={920} onClose={onClose}
      title={
        <span>{isEdit ? 'Modifier l\'agent' : 'Inviter un nouvel agent'}
          <span style={{color:'var(--ink-400)', fontWeight:400, fontSize:'.85em', marginLeft:6}}>/ {isEdit ? 'Edit agent' : 'New agent'}</span>
        </span>
      }
        ...
        <div>
          ...
          <div className="field-row field-row--2">
            ...
            <div className="field">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: data.active ? 'var(--ok-50)' : 'var(--bg-soft)', borderRadius: 7, border: '1px solid ' + (data.active ? 'var(--ok-100)' : 'var(--border)') }}>
                <span className="dot-status dot-status--ok" style={{ background: data.active ? 'var(--ok-500)' : 'var(--ink-300)' }} />
                <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: data.active ? 'var(--ok-700)' : 'var(--ink-500)' }}>
                  {data.active ? 'Actif · peut se connecter' : 'Suspendu · accès bloqué'}
                </span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 16 }}>
            {[
              { id: 'admin',    label: 'Administrateur',   desc: 'Accès complet à toute la plateforme',   icon: '👑', count: 25 },
              { id: 'agent',    label: 'Agent opérationnel', desc: 'Gestion des cargaisons et clients',   icon: '🚚', count: 13 },
              ...
              return (
                <div key={m.id} style={{ borderBottom: mi < PERMISSION_MODULES.length - 1 ? '1px solid var(--border-soft)' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--bg-soft)' }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--ink-500)', fontFamily: 'var(--ff-mono)' }}>{enabled}/{total}</span>
                  </div>
                ...
                <div style={{ padding: 14 }}>
                  <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 8, padding: 12 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--ink-700)', lineHeight: 1.5 }}>
                      ...
                      <a style={{ color: 'var(--brand-700)', fontWeight: 600, textDecoration: 'underline' }}>👉 Créer mon mot de passe</a>
                ...
                {PERMISSION_MODULES.map(m => {
                  ...
                  return (
                    <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
                      {Ic && <Ic style={{ width: 12, height: 12, color: 'var(--ink-400)', flex: '0 0 12px' }} />}
                      <span style={{ flex: 1, color: 'var(--ink-700)' }}>{m.label}</span>
                      {granted.length === m.perms.length ? <span style={{ color: 'var(--ok-600)', fontWeight: 700, fontSize: 11 }}>✓ tout</span>
                       : granted.length === 0 ? <span style={{ color: 'var(--ink-300)', fontSize: 11 }}>—</span>
...
window.AgentFormModal = AgentFormModal;
window.PERMISSION_MODULES = PERMISSION_MODULES;
