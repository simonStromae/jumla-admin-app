// ============================================
// ZENDIT — Screen: Analytics / Tableau de bord
// ============================================
  ...
  return (
    <div className="page">
        ...
        <div className="page__actions">
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 6px 4px 10px', border: '1px solid var(--border)', borderRadius: 8, background: 'white', fontSize: 12.5 }}>
            ...
            <select value={routeFilter} onChange={e => setRouteFilter(e.target.value)} style={{ border: 0, background: 'transparent', fontWeight: 600, paddingRight: 4 }}>
              ...
              {window.DATA.ROUTES.map(r => <option key={r.id} value={r.id}>{r.code}</option>)}
            </select>
          </div>
          <div className="tabs" style={{ padding: 2 }}>
            <button className={'tab '+(period==='ytd'?'is-active':'')} onClick={()=>setPeriod('ytd')} style={{padding:'4px 10px', fontSize:11.5}}>YTD</button>
            <button className={'tab '+(period==='month'?'is-active':'')} onClick={()=>setPeriod('month')} style={{padding:'4px 10px', fontSize:11.5}}>Mois</button>
            <button className={'tab '+(period==='12m'?'is-active':'')} onClick={()=>setPeriod('12m')} style={{padding:'4px 10px', fontSize:11.5}}>12 mois</button>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 18 }}>
        <KpiCard
          label="CA encaissé" en="Revenue"
          value={(totalColl/1000).toFixed(1)+'k'} unit="CAD"
          delta={18} deltaLabel="vs 2025"
          spark={[12,18,22,28,42,38,46,52,60,55,68,79]}
          color="var(--ok-500)"
          big
        />
        <KpiCard
          label="Taux recouvrement" en="Recovery"
          value={recoveryRate} unit="%"
          delta={2.1}
          progress={recoveryRate}
          color="var(--brand-500)"
        />
        <KpiCard
          label="Colis livrés" en="Parcels"
          value={totalParcels.toLocaleString('fr')} unit=""
          delta={12}
          spark={[28,35,42,38,52,48,62,68,72,68,80,88]}
          color="var(--info-500)"
        />
        <KpiCard
          label="Poids transporté" en="Weight"
          value={(totalWeight/1000).toFixed(1)} unit="t"
          delta={9}
          spark={[1.1,1.4,1.5,1.6,1.9,1.8,2.2,2.4,2.5,2.4,2.8,3.0]}
          color="var(--brand-500)"
        />
        <KpiCard
          label="Impayés" en="Outstanding"
          value="28,1k" unit="CAD"
          delta={-15} deltaLabel="vs T1"
          deltaInverse
          color="var(--bad-500)"
          sub="12 colis · 8 clients"
        />
      </div>

      {/* Main row: revenue chart + recovery */}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 14 }}>
        <RankingCard
          title="Top clients"
          sub="Par chiffre d'affaires YTD"
          icon={<I.Star style={{ color: 'var(--brand-500)' }} />}
          items={[
        ...
        <RankingCard
          ...
          sub="Villes au Canada"
          icon={<I.Pin style={{ color: 'var(--info-500)' }} />}
          items={[
        ...
        <RankingCard
          title="Top agents"
          sub="Par colis traités YTD"
          icon={<I.Users style={{ color: 'var(--ok-500)' }} />}
          items={[
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14 }}>
        <ChartCard title="Impayés à relancer" sub="12 colis · 28 140 CAD" actions={<a style={{ fontSize: 12, color: 'var(--brand-700)', fontWeight: 600, cursor: 'pointer' }} onClick={() => onNav('/payments')}>Voir tout →</a>}>
            ...
            <tbody>
              {[
                ...
                <tr key={i}>
                  ...
                  <td>
                    <span className={'badge badge--'+r.sev+' badge--dot'}>{r.age}</span>
                  </td>
        ...
        <ChartCard title="Activité récente" sub="Dernières actions de l'équipe">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { who: 'Aïcha M.', what: 'a clôturé', target: 'DLA-YUL-MAR-02', time: 'il y a 12 min', color: 1, kind: 'campaign' },
              { who: 'Marc L.',  what: 'a validé le bordereau',  target: 'BL-2604-05', time: 'il y a 1 h',  color: 2, kind: 'slip' },
              { who: 'Aïcha M.', what: 'a encaissé', target: '540 CAD de Client N', time: 'il y a 2 h',  color: 1, kind: 'payment' },
              { who: 'Tunde A.', what: 'a créé', target: '14 colis sur LOS-YUL-MAY-01', time: 'il y a 4 h',  color: 4, kind: 'parcel' },
              { who: 'Marc L.',  what: 'a envoyé un message à', target: '12 destinataires', time: 'il y a 5 h',  color: 2, kind: 'message' },
              { who: 'Aïcha M.', what: 'a créé la route', target: 'DLA → CDG', time: 'hier',  color: 1, kind: 'route' },
            ].map((a, i) => (
...
// === Revenue chart (bars with overlay) ===
    ...
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ width: '100%', height: 200 }}>
        ...
        {months.map((m, i) => {
          ...
          return (
            <g key={i}>
              <rect x={x} y={h - hRev} width={barW} height={hRev || 0.1}
                fill="var(--brand-100)" rx={1.4} vectorEffect="non-scaling-stroke"/>
              <rect x={x} y={h - hColl} width={barW} height={hColl || 0}
                fill="var(--brand-500)" rx={1.4} vectorEffect="non-scaling-stroke"/>
            </g>
          );
        })}
      </svg>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
      <svg viewBox="-70 -70 140 140" style={{ width: 130, height: 130, flexShrink: 0 }}>
        {data.map((d, i) => {
          ...
          const d_ = `M ${x1} ${y1} A ${R} ${R} 0 ${large} 1 ${x2} ${y2} L ${xi2} ${yi2} A ${r} ${r} 0 ${large} 0 ${xi1} ${yi1} Z`;
          return <path key={i} d={d_} fill={d.color} stroke="white" strokeWidth="1.5"/>;
        })}
      ...
      {routes.map(r => {
        ...
        return (
          <div key={r.id}>
            ...
            <div style={{ height: 8, background: 'var(--ink-100)', borderRadius: 999, overflow: 'hidden', position: 'relative' }}>
