// ============================================
// ZENDIT — Screen: Cargaisons (Dashboard)
// ============================================

function CampaignsScreen({ onNav, onNewCampaign }) {
  const [year, setYear] = useState(2026);
  const [filter, setFilter] = useState('all');
  const [routeFilter, setRouteFilter] = useState('all');
  const [view, setView] = useState('grid');

  const campaigns = window.DATA.CAMPAIGNS.filter(c => routeFilter === 'all' || c.route === routeFilter);
    ...
    <div className="page">
      ...
      <div className="page__head">
        ...
        <div className="page__actions">
          ...
          <button className="btn btn--brand" onClick={onNewCampaign}>
            <I.Plus />Nouvelle cargaison
          </button>
        </div>
      </div>
      ...
      {Object.entries(byMonth).map(([month, list]) => (
        <div key={month} style={{ marginBottom: 26 }}>
          <div style={{
            ...
            <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--ink-800)', letterSpacing:'-.01em' }}>
              {month}
            </h3>
    ...
    <div className="card" style={{ padding: 0, cursor: 'pointer', transition: 'all .15s', position: 'relative', overflow: 'hidden' }}
      ...
      <div style={{ padding: '16px 16px 12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 4 }}>
          <div className="mono" style={{ fontSize: 15, fontWeight: 700, letterSpacing: '-.01em', color: 'var(--ink-900)' }}>
            {c.code}
          </div>
        </div>
