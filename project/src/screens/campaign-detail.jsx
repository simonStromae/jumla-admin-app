// ============================================
// ZENDIT — Screen: Campaign detail / Parcels list
// ============================================

function CampaignDetailScreen({ id, onNav, onEdit, onClose: onCloseCampaign }) {
  const c = window.getCampaign(id) || window.DATA.CAMPAIGNS[0];
  const r = window.getRoute(c.route);
  ...
  // Map campaign.status to workflow id
  const workflowStatus = c.status === 'closed' ? 'arrived' : c.status === 'in-transit' ? 'in-transit' : c.status === 'processing' ? 'open' : 'open';
  // Lock when status is locked or beyond
  const lockedStates = ['locked', 'in-transit', 'arrived', 'closed'];
  const isLocked = lockedStates.includes(workflowStatus) || c.status === 'closed';
  const parcels = window.DATA.PARCELS;
  const pct = Math.round(c.collected / c.invoiced * 100);

  const tabs = [
    { id: 'all',     label: 'Tous',           en: 'All',         n: parcels.length },
    { id: 'unpaid',  label: 'Impayés',        en: 'Unpaid',      n: 2, cls: 'bad' },
    { id: 'todeliv', label: 'À livrer',       en: 'To deliver',  n: 5 },
    { id: 'noslip',  label: 'Sans bordereau', en: 'No slip',     n: 2, cls: 'warn' },
    { id: 'overrun', label: 'Dépassement',    en: 'Overrun',     n: 3, cls: 'warn' },
  ];
    ...
    <div className="page">
      ...
      <div className="page__head" style={{ marginBottom: 16 }}>
        <div style={{ flex: 1 }}>
          ...
          <div className="page__sub">
            ...
            <span style={{ marginLeft: 12, color: 'var(--ink-300)' }}>·</span>
            <span style={{ marginLeft: 12 }}>Dernière modif. par <strong style={{color:'var(--ink-700)'}}>Marc L.</strong> il y a 12 min</span>
          </div>
        </div>
        <div className="page__actions">
          ...
          {c.status !== 'closed' && <button className="btn btn--primary" onClick={onCloseCampaign}><I.Check />Clôturer</button>}
      ...
      <div style={{
        ...
        {[
          ...
          <div key={i} style={{
            padding: '14px 18px',
            borderRight: i < 5 ? '1px solid var(--border-soft)' : 'none',
          }}>
      ...
      <StatusPanel
        ...
        onAdvance={() => {
          const idx = window.STATUS_STEPS.findIndex(s => s.id === workflowStatus);
          const next = window.STATUS_STEPS[idx + 1];
      ...
      {isLocked && (
        <div style={{
          ...
          background: workflowStatus === 'closed' ? 'var(--bg-soft)' : 'var(--warn-50)',
          border: '1px solid ' + (workflowStatus === 'closed' ? 'var(--border)' : 'var(--warn-100)'),
          borderRadius: 10,
        }}>
          <div style={{
            ...
            color: workflowStatus === 'closed' ? 'var(--ink-500)' : 'var(--warn-700)',
          }}>
            <I.Lock />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-800)' }}>
              {workflowStatus === 'closed' ? 'Cargaison clôturée — lecture seule' :
               workflowStatus === 'arrived' ? 'Cargaison arrivée — modifications limitées' :
               workflowStatus === 'in-transit' ? 'Cargaison en transit — modifications bloquées' :
               'Cargaison verrouillée — plus de modifications possibles'}
            </div>
            <div style={{ fontSize: 12, color: 'var(--ink-500)', marginTop: 2 }}>
              {workflowStatus === 'arrived'
                ? 'Vous pouvez encore valider les bordereaux et enregistrer les paiements. Les poids et contenus sont figés.'
                : workflowStatus === 'closed'
                ? 'Aucune modification possible. La cargaison est archivée.'
                : 'Plus d\'ajout ni de modification de colis. Pour rouvrir, revenez à l\'étape précédente via le sélecteur de statut.'}
            </div>
          </div>
          {workflowStatus !== 'closed' && (
      ...
      <div className="toolbar">
        ...
        <button className="btn btn--soft btn--sm"><I.Tag />Statut</button>
            ...
            <tr key={p.id}>
              ...
              <td>
                <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                  ...
                  <button className="icon-btn" title={isLocked ? 'Cargaison verrouillée — édition impossible' : 'Modifier'} onClick={() => !isLocked && setParcelModal({ mode: 'edit', parcel: p })} disabled={isLocked} style={isLocked ? { opacity: 0.3, cursor: 'not-allowed' } : {}}><I.Edit /></button>
...
// ===== Close campaign modal — financial summary =====
function CloseCampaignModal({ id, onClose, onConfirm }) {
  const c = window.getCampaign(id) || window.DATA.CAMPAIGNS[0];
  const r = window.getRoute(c.route);
  const collected = c.collected;
  const outstanding = c.invoiced - c.collected;
  const internalTotal = 6950;
  const margin = collected - internalTotal;
  const marginPct = Math.round(margin / collected * 100);
    ...
    <Modal width={780} onClose={onClose}
      ...
      <div style={{ display: 'flex', gap: 14, marginBottom: 18 }}>
        ...
        <div className="card" style={{ flex: 1, padding: 14, background: 'var(--ok-50)', borderColor: 'var(--ok-100)' }}>
          ...
          <div className="mono" style={{ fontSize: 22, fontWeight: 700, marginTop: 4, color: 'var(--ok-700)' }}>{collected.toLocaleString('fr')} <span style={{ fontSize: 12, opacity: .7 }}>CAD</span></div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, padding: 12, background: 'var(--warn-50)', border: '1px solid var(--warn-100)', borderRadius: 8, marginBottom: 18 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--ink-600)', marginTop: 2 }}>
              3 colis présentent des impayés ({outstanding.toLocaleString('fr')} CAD). Choisissez comment les traiter :
            </div>
            <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '5px 10px', background: 'white', border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer' }}>
                <input type="radio" name="outstanding" defaultChecked style={{accentColor:'var(--brand-500)'}}/> Reporter sur la prochaine cargaison
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '5px 10px', background: 'white', border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer' }}>
                <input type="radio" name="outstanding" style={{accentColor:'var(--brand-500)'}}/> Marquer comme perte
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, padding: '5px 10px', background: 'white', border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer' }}>
                <input type="radio" name="outstanding" style={{accentColor:'var(--brand-500)'}}/> Garder ouvert
              </label>
            </div>
          </div>
        </div>
        ...
        <table className="tbl tbl--compact" style={{ borderRadius: 0 }}>
          <tbody>
            ...
            <tr style={{ background: 'var(--ink-900)', color: 'white' }}>
              ...
              <td style={{ textAlign: 'right' }}>
                <span className="mono" style={{ fontWeight: 700, fontSize: 16, color: '#F5A524' }}>
                  +{margin.toLocaleString('fr')} CAD
                </span>
      ...
      <div style={{ display: 'flex', gap: 10, padding: 12, marginTop: 14, background: 'var(--info-50)', border: '1px solid var(--info-100)', borderRadius: 8 }}>
        ...
        <div style={{ fontSize: 12.5, color: 'var(--ink-700)', lineHeight: 1.5 }}>
          La clôture archive la cargaison en lecture seule, génère le rapport financier PDF et envoie un récap par WhatsApp à chaque destinataire (livraison + bilan).
        </div>
      </div>
    </Modal>
  );
