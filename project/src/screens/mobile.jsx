// ============================================
// ZENDIT — Mobile view (parcel list for field agent)
// ============================================

function MobileScreen({ onNav }) {
  ...
  return (
    <div style={{ background: '#E5E7EB', minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 24 }}>
      <div style={{
        ...
        {/* Status bar */}
        <div style={{
          height: 44, background: 'var(--bg-page)',
          padding: '0 24px',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            <svg width="16" height="11" viewBox="0 0 16 11"><path fill="#000" d="M.5 8h2v3h-2zM4 6h2v5H4zM7.5 4h2v7h-2zM11 2h2v9h-2zM14.5 0h2v11h-2z"/></svg>
            <svg width="14" height="11" viewBox="0 0 14 11"><path fill="#000" d="M7 2a8 8 0 0 1 5.6 2.3l-1 1A6.6 6.6 0 0 0 7 3.4 6.6 6.6 0 0 0 2.4 5.4l-1-1A8 8 0 0 1 7 2zm0 3a4.9 4.9 0 0 1 3.5 1.4l-1 1A3.5 3.5 0 0 0 7 6.5a3.5 3.5 0 0 0-2.5 1l-1-1A4.9 4.9 0 0 1 7 5zm0 3a2 2 0 0 1 1.4.6L7 10 5.6 8.6A2 2 0 0 1 7 8z"/></svg>
            <svg width="24" height="11" viewBox="0 0 24 11"><rect x=".5" y=".5" width="21" height="10" rx="2.5" fill="none" stroke="#000"/><rect x="2" y="2" width="14" height="7" rx="1" fill="#000"/><rect x="22" y="3.5" width="1.5" height="4" rx=".5" fill="#000"/></svg>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 18px 12px', borderBottom: '1px solid var(--border)', background: 'white' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button className="icon-btn"><I.ArrowLeft /></button>
            <div style={{ flex: 1 }}>
          ...
          {parcels.slice(0, 5).map(p => (
            <div key={p.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <span className={'badge badge--dot badge--'+window.STATUS.payment[p.paid].cls} style={{ fontSize: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11, color: 'var(--ink-500)', paddingTop: 8, borderTop: '1px solid var(--border-soft)', marginBottom: 8 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }} className="mono">
                  <I.Scale style={{width:11, height:11}}/> {p.actualKg} kg{p.overrun && <span style={{color:'var(--warn-700)'}}> ⚠</span>}
                </span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                  {p.delivery === 'home' ? <><I.Truck style={{width:11, height:11}}/> Domicile</> : <><I.Warehouse style={{width:11, height:11}}/> Retrait</>}
                </span>
                {p.slip && <span className="mono" style={{ marginLeft: 'auto', fontSize: 10, color: 'var(--ink-400)' }}>{p.slip}</span>}
              </div>

              <div style={{ display: 'flex', gap: 6 }}>
                <button style={{ flex: 1, padding: '7px 10px', fontSize: 11.5, fontWeight: 600, borderRadius: 7, background: 'var(--ok-50)', color: 'var(--ok-700)', border: '1px solid var(--ok-100)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                  <I.Whatsapp style={{width:13, height:13}}/> WhatsApp
                </button>
                {p.paid !== 'paid' && (
                  <button style={{ flex: 1, padding: '7px 10px', fontSize: 11.5, fontWeight: 600, borderRadius: 7, background: 'var(--brand-500)', color: 'white', border: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                    <I.Check style={{width:13, height:13}}/> Marquer payé
                  </button>
                )}
                {p.paid === 'paid' && (
                  <button style={{ flex: 1, padding: '7px 10px', fontSize: 11.5, fontWeight: 600, borderRadius: 7, background: 'var(--bg-soft)', color: 'var(--ink-700)', border: '1px solid var(--border)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
